...
//			}
			...
			if (value==7)
				return recordOfLosses(losses, steps);
			else if (value==point)
				return recordOfWins(wins, steps, value);
		}

		return false;
	}

	private boolean recordOfWins(int[] wins, int steps, int value)
	{
		{
	...
	private boolean recordOfLosses(int[] losses, int steps)
	{
		{
